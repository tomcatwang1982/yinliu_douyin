...
try :
    for j in range(1,loop_video_num):
        ...
        ##get ping lun shu
        try :
            if not d(resourceId='com.ss.android.ugc.aweme:id/a50').exists(timeout=3):
                tap(442, 413)
        ...
        if (int_ping_lun_total > 0) :
            ...
            for i in range(1,loop_message_num) :
                try :
                    print('---------------------i='+str(i))
                    #if_continue = False
                    if d(resourceId="com.ss.android.ugc.aweme:id/a81",index=str(i)).exists(timeout=2) :
