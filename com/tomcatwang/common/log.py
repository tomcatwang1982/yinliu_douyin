...
class Logger:
    def __init__(self, log_path,set_level="INFO",
        ...
        """
        log_path_string = os.path.join(log_path, "log")
        if not set_level:
        ...
        if not os.path.exists(log_path_string):  # 创建日志目录
            os.makedirs(log_path_string)
        formatter = logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
        handler_list = list()
        handler_list.append(logging.FileHandler(os.path.join(log_path_string, log_name), encoding="utf-8"))
        if use_console:
